      ...
      <Tabs defaultValue="overview" className="space-y-4">
        ...
        <TabsContent value="hardware" className="space-y-4">
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <Info className="h-5 w-5" /> Resumo do Hardware
              </CardTitle>
            </CardHeader>
            <CardContent className="grid grid-cols-2 gap-3 text-sm">
              <p><strong>CPU:</strong> {hardware.cpu_info?.model || 'N/A'}</p>
              <p><strong>RAM:</strong> {safeNumber(hardware.ram_info?.total_gb)} GB</p>
              <p><strong>Disco:</strong> {hardware.disk_info.length > 0
                ? `${hardware.disk_info.reduce((acc, d) => acc + safeNumber(d.total_gb), 0)} GB`
                : 'N/A'}</p>
              <p><strong>GPU:</strong> {hardware.gpu_info?.model || 'N/A'}</p>
              <p><strong>Rede:</strong> {hardware.network_info[0]
                ? `${hardware.network_info[0].name} (${hardware.network_info[0].ip_addresses?.[0] || 'Sem IP'})`
                : 'N/A'}</p>
            </CardContent>
          </Card>
          ...
          {/* Detalhes só aparecem se expandir */}
          {showDetails && (
            <div className="space-y-4">
                ...
                {hardware.disk_info.map((disk, index) => {
                  ...
                  return (
                    <div key={index} className="p-3 border rounded-lg mb-2">
                      <p className="font-medium">{disk.model || disk.name || 'N/D'} ({disk.type || 'N/D'})</p>
              ...
              {/** Rede */}
              <ExpandableCard title="Interfaces de Rede" icon={<Wifi className="h-5 w-5 text-green-600" />}>
                {hardware.network_info.map((net, i) => (
                  <div key={i} className="border-b py-1 text-sm">
                    <p><strong>Nome:</strong> {net.name}</p>
                    <p><strong>MAC:</strong> {net.mac}</p>
                    <p><strong>IPs:</strong> {Array.isArray(net.ip_addresses) ? net.ip_addresses.join(", ") : "N/A"}</p>
                  </div>
                ))}
              </ExpandableCard>
              ...
              {hardware.power_supply_info && (
                <ExpandableCard title="Fonte de Alimentação" icon={<Activity className="h-5 w-5 text-orange-600" />}>
                  <p>Status: {hardware.power_supply_info.status || 'N/A'}</p>
                  <p>Consumo: {safeNumber(hardware.power_supply_info.consumption_watts)} W</p>
                </ExpandableCard>
              )}
            </div>
          )}
